...
impl Auction {
    ...
    pub fn submit_enc_bid(
        ...
        if condition == c {
            ...
            inner_vec.hash.set_str(hash.clone());
    ...
    pub fn submit_decrypted(&mut self, data: Vec<String>) -> Result<bool, AuctionError >{
        ...
        if sha256_of_string(&data[i]).eq(&self.bids.get_mut(i).unwrap().hash.get_string()){
            self.bids.get_mut(i).unwrap().plain_tx.set_str(data[i].clone());
