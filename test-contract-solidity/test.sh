...
OUTPUT=$(forge create --rpc-url $RPC_URL --private-key $PRIVATE_KEY test-contract-solidity/Encrypted.sol:MessageStorage --constructor-args $ADDRESS).await
...
cast call --rpc-url $RPC_URL --private-key $PRIVATE_KEY $Contract "checkMessages(string)(uint8[][])" "test"